...
/**
 * کارتابل بررسی درخواست‌ها برای بخش مالی یا منابع انسانی.
 */
export default function RequestsInbox({ scope, title }) {
  ...
  const load = useCallback(async () => {
    const params = new URLSearchParams({ page: String(page), scope });
    if (status) params.set('status', status);
    const d = await api(`/requests?${params.toString()}`);
    setData(d);
  }, [page, status, scope]);
  ...
  async function decide(approved) {
    ...
    setError('');
    try {
      await api(`/requests/${selected._id || selected.id}/decide`, {
        method: 'PATCH',
        body: { status: approved ? 'APPROVED' : 'REJECTED', note },
      });
      setSelected(null);
      setNote('');
      await load();
          ...
          <div className="space-y-4">
            ...
            {selected.status === 'PENDING' ? (
              ...
              <div className="text-sm text-slate-500">
                ...
                {selected.decisionNote && <p className="mt-2">یادداشت: {selected.decisionNote}</p>}
              </div>
            )}
          </div>
        )}
